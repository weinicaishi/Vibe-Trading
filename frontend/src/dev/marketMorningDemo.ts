...
export const MARKET_MORNING_DEV_API_PROXY_PATTERN =
  "^/market-morning/(?:_internal/(?:deployment-preflight|operations/(?:summary|halts(?:/[^/]+)?)|event-briefs(?:/[^/]+/review)?|content-reports(?:/[^/]+/review)?|issuer-aliases(?:/[^/]+/review)?|event-merge-candidates(?:/[^/]+/review)?|private-beta/invites(?:/[^/]+)?|users/[^/]+/access)|private-beta/invitations/accept|delivery-links/redeem|settings|edition/today|editions/[^/]+/(?:event-states|events/[^/]+/(?:state|report)|sources/open)|watchlist(?:/[^/]+)?|issuers/(?:search|[^/]+/(?:research|note))|consents|account-deletion-requests|account-data-export)(?:\\?[^#]*)?$";
