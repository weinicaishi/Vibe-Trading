...
`6363 passed, 25 skipped`，无失败或错误；25 项 skip 均有登记的外部前置条件。该本机结果来自
不包含真实 `.env` 的干净 Git 快照；macOS 开启系统 HTTP 代理时，测试命令需要为
`127.0.0.1,localhost` 设置 `NO_PROXY`，避免 Python `requests` 把 MCP 回环请求交给系统代理。
上一版 release CI 基线为 `6360 passed, 25 skipped`，新增 3 项来自 Alpha A 启动器测试；最终远端
数字仍以同 revision 的最新 release candidate CI 为准。CI 环境变量 Gate 与语法检查已通过。这些结果仍不能替代真实 MySQL、
