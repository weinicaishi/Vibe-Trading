...
- Market Morning 普通后端回归：直接按 `agent/tests/test_market_morning_*.py` 运行得到 `931 passed, 15 skipped`；15 项仅因普通命令未显式授权两套专用数据库写入。专用远程 MySQL 8 的业务 acceptance 为 `12/12 passed`，其中 schema/session 场景已覆盖 access-token hash 的幂等首见、精确注销、不可复活与多 token 隔离；destructive migration 为 `3/3 passed`。两份 manifest 均为 `environment=ci` 且固定不计 staging/T1。
...
- Vibe-Trading 整仓后端：上一版 release CI 在不包含任何真实 `.env` 的干净 runner 中运行 `agent/tests`，得到 `6360 passed, 25 skipped`、无失败或错误；本轮包含 Alpha A 启动器新增测试的干净 Git 快照在相同排除范围下得到 `6363 passed, 25 skipped`、无失败或错误。25 项 skip 均为已登记的 MySQL／合成因子／Tushare 前置条件。macOS 开启系统 HTTP 代理时，Python `requests` 的本机 MCP 回环测试必须显式让 `127.0.0.1,localhost` 走 `NO_PROXY`；不设置时会由系统代理返回 502，直连服务本身已验证为 200。CI 环境变量 Gate 与语法检查通过。
- T0 合成演练：12/12 通过，证据位于 `docs/evidence/market-morning/t0-local-synthetic-2026-07-21.json`，固定不计为 staging 日。
- 前端：347 项回归、TypeScript 与 production build 通过；同一不可变镜像在挂载 React 前从后端读取公开 Auth0 SPA 配置，production 缺失/非法时只关闭 Market Morning，development 才允许回退 `VITE_*`。带四个构建期 sentinel 的产物扫描证明 Auth0 domain/audience/Product/Operator Client ID 均未进入 bundle。覆盖 product/operator lifecycle 并发初始化、官方 Auth0 SDK 配置/PKCE/memory cache/callback 清理/revoke/logout、无 bearer 的无凭据开发诊断、401、初始化失败重试、邮件深链认证后兑换时序，以及 Demo 只能在 development 启用、状态变更和完整 API 代理路径。产品边界现在会在 SDK 初始化后先确认内存 Bearer，未登录时直接显示登录 Gate，不挂载 `MarketMorningProvider` 或请求 settings/watchlist；真实浏览器复验仅记录 `not_authenticated`，不再产生这两个 401。
...
- Alpha A 本地真实 OIDC：Product/Operator 均使用新签发的 900 秒 Auth0 access token；浏览器虚拟推进 16 分钟后都取得不同的新 token，刷新后的受保护 API 为 200。Product 已把 7203 写入独立 staging MySQL，并由第二个独立 token 会话读到同一关注股；精确注销后旧 Product/Operator token 均为 401，未注销的另一个 Product token 仍为 200。启动器现在可只读前端已有的五项公开 Auth0 值并仅注入后端子进程；实际启动后后端及 Vite 代理的 `/market-morning/runtime-config` 均返回 `ready`、无阻断码，浏览器不再因代理缺路由而回退。去敏证据见 `alpha-a-oidc-local-2026-07-23.json`；它不保存 token、OAuth code、邀请码或身份信息，并固定 `counts_as_staging_day=false`、`counts_as_t1_release_evidence=false`。
- 不可变发布候选：固定源文件现为 25 项；除 Auth0 Post-Login Action、900 秒部署合同、OIDC staging 探针/CLI 与默认 blocked 模板外，后端公开 runtime config、环境 schema、前端 bootstrap/runtime loader/Auth0 adapter 也全部纳入 SHA-256 绑定。GitHub Actions run `29993371107` 已对 revision `c45fe7fcc05b915a6e4b2c8ce73c5728ec6d09fe` 独立复验：九项检查全 passed、`blocking_codes=[]`、schema 为 `0018_market_morning_auth_sessions`、25 份源码 hash 与 5 份验证证据完整。远端发布线必须继续对每个最终 revision 独立复验，本地 manifest 不替代 CI；两类候选均固定 `counts_as_staging_day=false`、`counts_as_t1_release_evidence=false`。实时有效 revision 以 `release/market-morning-mvp` 的最新成功 CI artifact 为准。
...
1. 运行时公开配置代码、开发线／release 线同步和不可变候选 CI 已完成。下一步是在独立非 localhost HTTPS Staging 部署最新成功 manifest 绑定的同一 revision，并为该环境注入五项 `VIBE_MARKET_MORNING_PUBLIC_*` 公开 Auth0 配置；先确认 `/market-morning/runtime-config` 为 `ready`、deployment preflight 的 `frontend_runtime_configured=true`，再使用 owner-only token 文件运行自动 OIDC 探针，复验 Alpha A 已通过的 900 秒刷新、角色、精确注销和 Product 跨会话关注股。该步骤才可生成可计数的 `oidc_session` Gate。
